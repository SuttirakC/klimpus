    ...
    <>
      ...
      {/* <div className="flex flex-wrap mt-8 justify-center">
        {notifications.map((noti, index) => (
          <div key={index} className="w-full max-w-60 px-4">
            <CardDashboard
              bgcolor={getBackgroundColor(noti.noti_case_status)}
              statSubtitle={noti.noti_system_name}
              statTitle={noti.noti_case_status === 1 ? "Error" : "Normal"}
              statIconName={getIconName(noti.noti_case_status)}
              statIconColor="bg-gray-300"
              statStatus={getStatusIcon(noti.noti_case_status)}
            />
          </div>
        ))}
      </div> */}
