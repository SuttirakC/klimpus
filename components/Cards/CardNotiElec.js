...
export default function CardNotiElec({ notifications }) {
    return (
        <>
                ...
                <div className="flex-auto px-4 lg:px-10 py-10 pt-0">
                    {notifications.length > 0 ? (
                        notifications.map((noti, index) => (
                            <div key={index} role="alert" className={`mt-4 alert bg-kmutt_${noti.noti_case_level === 1 ? 'yellow' : 'red'}-100`}>
...
// export default function CardNotiElec() {
//     return (
//         <>
...
//                             <span>Error! LP13 on the first floor wasn&apos;tworking.</span>
...
//                             <span>Warning: There was a power outage.</span>
//                             {/* {new Date().getDate()}-{new Date().getMonth()}-{new Date().getFullYear()}{"  "}{new Date().getHours()}:{new Date().getMinutes()}:{new Date().getSeconds()} */}
//                         </div>
